...
import { Pill, Plus, Clock, Trash2, Calendar, Bell, CheckCircle, Users, Package, TimerReset, Mail, Smartphone } from 'lucide-react'
...
export default function Medications() {
  ...
  async function handleAddMedication() {
    ...
    try {
      ...
      alert(error.message || 'Erro ao adicionar medicamento. Rode SQL_FAMILIA_IDOSOS_FASE1.sql no Supabase se ainda não rodou.')
    }
  }
    ...
    <div className="space-y-5 pb-28">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {activeMeds.length > 0 && <MedGroup title={`Em uso (${activeMeds.length})`} meds={activeMeds} savingId={savingId} onConfirm={confirmMedication} onCalendar={openCalendarReminder} onToggle={handleToggleActive} onDelete={handleDelete} />}
          {inactiveMeds.length > 0 && <MedGroup title={`Inativos (${inactiveMeds.length})`} muted meds={inactiveMeds} savingId={savingId} onConfirm={confirmMedication} onCalendar={openCalendarReminder} onToggle={handleToggleActive} onDelete={handleDelete} />}
        </div>
      )}
...
function MedGroup({ title, meds, muted, savingId, onConfirm, onCalendar, onToggle, onDelete }: any) {
  return <div className={muted ? 'opacity-70' : ''}><h2 className="text-sm font-semibold mb-2">{title}</h2><div className="space-y-2">{meds.map((med: Medication) => <MedicationCard key={med.id} med={med} active={med.is_active} saving={savingId === med.id} onConfirm={(status: 'taken' | 'delayed' | 'skipped') => onConfirm(med, status)} onCalendar={() => onCalendar(med)} onToggle={() => onToggle(med.id, med.is_active)} onDelete={() => onDelete(med.id)} />)}</div></div>
}

function MedicationCard({ med, active, saving, onConfirm, onCalendar, onToggle, onDelete }: any) {
    ...
    <div className="bg-card rounded-xl border border-border p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap"><p className="font-semibold">{med.name}</p>{med.critical_medication && <Badge text="Crítico" tone="red" />}</div>
          ...
          {med.reminder_time && <button type="button" onClick={onCalendar} className="mt-2 w-full rounded-lg border border-blue-200 py-2 text-xs font-semibold text-blue-700 flex items-center justify-center gap-1"><Calendar className="w-3 h-3" /> Abrir agenda/alarme</button>}
        </div>
...
function openCalendarReminder(med: any) {
  ...
  const time = String(med.reminder_time || '09:00').slice(0, 5).replace(':', '')
  const date = (med.start_date || today).replace(/-/g, '')
  const title = encodeURIComponent(`Tomar ${med.name}`)
  const details = encodeURIComponent(`${med.dosage || ''} ${med.frequency || ''}`.trim())
  const url = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${title}&dates=${date}T${time}00/${date}T${time}00&details=${details}`
  window.open(url, '_blank')
}

function Stat({ label, value }: any) { return <div className="bg-white rounded-xl border p-3 text-center"><p className="text-2xl font-bold text-orange-700">{value}</p><p className="text-xs text-muted-foreground">{label}</p></div> }
function Badge({ text, tone = 'emerald' }: any) { const cls = tone === 'red' ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'; return <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${cls}`}>{text}</span> }
...
function NumberInput({ label, value, onChange }: any) { return <div><label className="text-xs font-medium mb-1 block">{label}</label><input type="number" min="0" value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-3 py-3 rounded-xl border border-border bg-background" /></div> }
...
function normalizeFrequency(value: string) { const text = (value || '').toLowerCase(); if (text.includes('2x') || text.includes('duas') || text.includes('12')) return 'twice_daily'; if (text.includes('8 em 8') || text.includes('3x')) return 'three_times_daily'; if (text.includes('semana')) return 'weekly'; if (text.includes('mês') || text.includes('mes')) return 'monthly'; return 'daily' }
function calculateStockDays(stock?: number | null, perDay?: number | null) { if (!stock || !perDay || Number(perDay) <= 0) return null; return Math.floor(Number(stock) / Number(perDay)) }
